...
app.config['MYSQL_PASSWORD'] = os.environ.get('MYSQL_PASSWORD', 'epinfo123')
app.config['MYSQL_DB']       = os.environ.get('MYSQL_DB', 'db_EPI')
...
def add_funcionario():
    ...
    cur.execute(
        "INSERT INTO funcionario (nome, email, departamento, senha) VALUES (%s, %s, %s, %s)",
...
def login():
    ...
    cur = mysql.connection.cursor()
    cur.execute(
        "SELECT * FROM funcionario WHERE (nome = %s OR email = %s) AND senha = %s",
    ...
    if usuario:
        return jsonify({
            ...
            'usuario': {'nome': usuario[1], 'email': usuario[2], 'departamento': usuario[3]}
        })
    else:
